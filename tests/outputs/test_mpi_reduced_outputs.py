...
def compare_group(serial, distributed, precision, *, exact_float=False):
    assert set(serial.keys()) == set(distributed.keys())
    for name in serial:
    ...
    for name in serial.attrs:
        ...
        if left.dtype.kind in "fc":
            np.testing.assert_allclose(
                right, left, rtol=1e-6, atol=1e-25, equal_nan=True, err_msg=name
            )
        else:
...
def test_reduced_sar_radiometry_and_snapshots_match_serial(tmp_path, mode, split, precision):
    ...
    with h5py.File(serial / "result.h5") as left, h5py.File(distributed / "result.h5") as right:
        for path in ("sar/dose", "radiometry/receive"):
            compare_group(left[path], right[path], precision)
