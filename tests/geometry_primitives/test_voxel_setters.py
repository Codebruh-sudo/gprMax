...
class TestBuildVoxelHard:
    ...
    def expected_id_writes(i, j, k):
        ...
        # Trailing-face writes added upstream: each E-component at the far
        # face of the voxel (xf for Ex, yf for Ey, zf for Ez).
        trailing = {
            0: i + 1,  # Ex at xf
            1: j + 1,  # Ey at yf
            2: k + 1,  # Ez at zf
        ...
        # Trailing-face writes for the magnetic components (3-5 mirror 0-2)
        for comp in range(3, 6):
            ec = comp % 3
            t = trailing[ec]
            cell = [i, j, k]
            cell[ec] = t
            expected[(comp, *cell)] = values[ec]
        return expected
    ...
    def test_far_corner_writes_into_id_padding(self, grid_arrays):
        ...
        written = nonzero_set(g.ID)
        assert len(written) == 18  # 6 components × 3 corners (upstream change)
        # The far-corner entries that exercise the padding still land.
