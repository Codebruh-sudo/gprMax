...
def _primitive(shape, material, averaging):
    ...
    if shape == "cylinder":
        return gprMax.Cylinder(p1=(0.007, 0.010, 0.010), p2=(0.013, 0.010, 0.010), r=0.0022, **kwargs)
    if shape == "cone":
        return gprMax.Cone(p1=(0.007, 0.010, 0.010), p2=(0.013, 0.010, 0.010), r1=0.0012, r2=0.0032, **kwargs)
    if shape == "triangle":
        return gprMax.Triangle(
            p1=(0.007, 0.007, 0.008), p2=(0.013, 0.007, 0.008), p3=(0.010, 0.014, 0.008), thickness=0.004, **kwargs
        )
    return gprMax.CylindricalSector(
        normal="z", ctr1=0.010, ctr2=0.010, extent1=0.008, extent2=0.012, r=0.0032, start=0, end=270, **kwargs
    )
...
def _solve(scene, path, captured, **options):
    gprMax.run(
        scenes=[scene], n=1, outputfile=path, hide_progress_bars=True, log_level=40, cpu_precision="double", **options
    )
    with h5py.File(path.with_suffix(".h5")) as output:
        traces = np.stack([output[f"rxs/rx1/{field}"][:] for field in FIELDS])
    grid = captured["grid"]
    return {name: getattr(grid, name).copy() for name in ("solid", "ID", "rigidE", "rigidH")}, traces
...
def test_transparent_full_component_import_preserves_rigid_geometry_and_fields(tmp_path, monkeypatch):
    captured = _capture(monkeypatch)
    ...
    for imported in (False, True):
        ...
        if imported:
            scene.add(gprMax.GeometryObjectsRead(p1=(0.006,) * 3, geofile=path, matfile=materials))
...
@pytest.mark.gpu
@pytest.mark.parametrize("shape", SHAPES)
def test_cuda_uploaded_overwrite_geometry_and_fields_match_cpu(tmp_path, monkeypatch, gpu_device, shape):
    ...
    actual, actual_fields = _solve(overwritten, tmp_path / "cuda", captured, gpu=[gpu_device], gpu_precision="double")
    assert uploads == [True]
    for name in expected:
        np.testing.assert_array_equal(actual[name], expected[name], err_msg=name)
