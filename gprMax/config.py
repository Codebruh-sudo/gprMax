...
class SimulationConfig:
    ...
    def __init__(self, args):
        ...
        # Precision (single/double) for all solvers. Default is single, which
        # provides best performance and is the norm for both Cython (CPU) and
        # GPU. Double is available for both and selected via the `double` flag
        # (CLI --double / API double=True); it is well established for CPU and
        # supported on GPUs with double-precision capability.
        precision = "double" if getattr(args, "double", False) else "single"
