    ...
    def update_plane_waves_magnetic(self, iteration):
        ...
        for dpw in self.grid.discreteplanewaves:
            ...
            if M_int > 0:
                ...
                itemsize = dpw.H_fields_dev.dtype.itemsize
                for comp in range(3):
                    ...
                    row_ptr = int(dpw.H_fields_dev.gpudata) + comp * dpw.H_fields_dev.strides[0]
    ...
    def update_plane_waves_electric(self, iteration):
        ...
        for dpw in self.grid.discreteplanewaves:
            ...
            if M_int > 0:
                ...
                itemsize = dpw.E_fields_dev.dtype.itemsize
                for comp in range(3):
                    proj = dpw.projections[comp]
                    src_vals = (proj * wave_E[comp]).astype(
                        config.sim_config.dtypes["float_or_double"]
                    )
                    row_ptr = int(dpw.E_fields_dev.gpudata) + comp * dpw.E_fields_dev.strides[0]
                    self.drv.memcpy_htod(row_ptr, src_vals)
