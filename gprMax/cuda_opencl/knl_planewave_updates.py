...
update_1d_electric_dispersive_A = {
    "args_cuda": Template(
        ...
        __global__ void __launch_bounds__(256, 4) update_1d_electric_dispersive_A(
            ...
            const $REAL coef_E_z,
            const $REAL coef_E_D,
            const $REAL* __restrict__ coef_D,
            $REAL* __restrict__ T_x,
            $REAL* __restrict__ T_y,
            $REAL* __restrict__ T_z,
            $REAL* __restrict__ E_x,
            $REAL* __restrict__ E_y,
            $REAL* __restrict__ E_z,
            const $REAL* __restrict__ H_x,
    ...
    "args_opencl": Template(
        ...
        const $REAL coef_E_z,
        const $REAL coef_E_D,
        __global const $REAL* restrict coef_D,
        __global $REAL* restrict T_x,
        __global $REAL* restrict T_y,
        __global $REAL* restrict T_z,
        __global $REAL* restrict E_x,
        __global $REAL* restrict E_y,
        __global $REAL* restrict E_z,
        __global const $REAL* restrict H_x,
    ...
    for (pole = 0; pole < num_poles; pole++) {
        phi += coef_D[pole * 3] * T_x[pole * N + j];
        T_x[pole * N + j] = coef_D[1 + pole * 3] * T_x[pole * N + j]
                          + coef_D[2 + pole * 3] * E_x[j];
    }
    ...
    phi = 0;
    for (pole = 0; pole < num_poles; pole++) {
        phi += coef_D[pole * 3] * T_y[pole * N + j];
        T_y[pole * N + j] = coef_D[1 + pole * 3] * T_y[pole * N + j]
                          + coef_D[2 + pole * 3] * E_y[j];
    }
    ...
    phi = 0;
    for (pole = 0; pole < num_poles; pole++) {
        phi += coef_D[pole * 3] * T_z[pole * N + j];
        T_z[pole * N + j] = coef_D[1 + pole * 3] * T_z[pole * N + j]
                          + coef_D[2 + pole * 3] * E_z[j];
    }
...
update_1d_electric_dispersive_B = {
    ...
    int pole;
    for (pole = 0; pole < num_poles; pole++) {
        T_x[pole * N + j] -= coef_D[2 + pole * 3] * E_x[j];
        T_y[pole * N + j] -= coef_D[2 + pole * 3] * E_y[j];
        T_z[pole * N + j] -= coef_D[2 + pole * 3] * E_z[j];
    }
    """
    ),
}
...
update_1d_electric_dispersive_axial_source = {
    "args_cuda": Template(
        ...
        __global__ void __launch_bounds__(256, 4) update_1d_electric_dispersive_axial_source(
            ...
            const int m_z,
            const int num_poles,
            $REAL* __restrict__ T_x_s,
            $REAL* __restrict__ T_y_s,
            $REAL* __restrict__ T_z_s,
            $REAL* __restrict__ E_x_s,
            $REAL* __restrict__ E_y_s,
            $REAL* __restrict__ E_z_s,
            const $REAL* __restrict__ H_x_s,
    ...
    "args_opencl": Template(
        ...
        const int m_z,
        const int num_poles,
        __global $REAL* restrict T_x_s,
        __global $REAL* restrict T_y_s,
        __global $REAL* restrict T_z_s,
        __global $REAL* restrict E_x_s,
        __global $REAL* restrict E_y_s,
        __global $REAL* restrict E_z_s,
        __global const $REAL* restrict H_x_s,
        ...
        __global const $REAL* restrict matD
        """
    ),
    "args_metal": Template(
        """
    ...
    phi = 0;
    for (pole = 0; pole < num_poles; pole++) {
        phi += matD[IDX2D_MATDISP(mat, pole * 3)] * T_x_s[pole * N + j];
        T_x_s[pole * N + j] = matD[IDX2D_MATDISP(mat, 1 + pole * 3)] * T_x_s[pole * N + j]
                            + matD[IDX2D_MATDISP(mat, 2 + pole * 3)] * E_x_s[j];
    }
    ...
    phi = 0;
    for (pole = 0; pole < num_poles; pole++) {
        phi += matD[IDX2D_MATDISP(mat, pole * 3)] * T_y_s[pole * N + j];
        T_y_s[pole * N + j] = matD[IDX2D_MATDISP(mat, 1 + pole * 3)] * T_y_s[pole * N + j]
                            + matD[IDX2D_MATDISP(mat, 2 + pole * 3)] * E_y_s[j];
    }
    ...
    phi = 0;
    for (pole = 0; pole < num_poles; pole++) {
        phi += matD[IDX2D_MATDISP(mat, pole * 3)] * T_z_s[pole * N + j];
        T_z_s[pole * N + j] = matD[IDX2D_MATDISP(mat, 1 + pole * 3)] * T_z_s[pole * N + j]
                            + matD[IDX2D_MATDISP(mat, 2 + pole * 3)] * E_z_s[j];
    }
...
update_1d_electric_dispersive_axial_source_T = {
    ...
    for (pole = 0; pole < num_poles; pole++) {
        T_x_s[pole * N + j] -= matD[IDX2D_MATDISP(mat_x, 2 + pole * 3)] * E_x_s[j];
        T_y_s[pole * N + j] -= matD[IDX2D_MATDISP(mat_y, 2 + pole * 3)] * E_y_s[j];
        T_z_s[pole * N + j] -= matD[IDX2D_MATDISP(mat_z, 2 + pole * 3)] * E_z_s[j];
    }
    """
    ),
}
...
update_1d_electric_dispersive_axial_main = {
    ...
    phi = 0;
    for (pole = 0; pole < num_poles; pole++) {
        phi += matD[IDX2D_MATDISP(mat, pole * 3)] * T_x[pole * N + j];
        T_x[pole * N + j] = matD[IDX2D_MATDISP(mat, 1 + pole * 3)] * T_x[pole * N + j]
                          + matD[IDX2D_MATDISP(mat, 2 + pole * 3)] * E_x[j];
    }
    ...
    phi = 0;
    for (pole = 0; pole < num_poles; pole++) {
        phi += matD[IDX2D_MATDISP(mat, pole * 3)] * T_y[pole * N + j];
        T_y[pole * N + j] = matD[IDX2D_MATDISP(mat, 1 + pole * 3)] * T_y[pole * N + j]
                          + matD[IDX2D_MATDISP(mat, 2 + pole * 3)] * E_y[j];
    }
    ...
    phi = 0;
    for (pole = 0; pole < num_poles; pole++) {
        phi += matD[IDX2D_MATDISP(mat, pole * 3)] * T_z[pole * N + j];
        T_z[pole * N + j] = matD[IDX2D_MATDISP(mat, 1 + pole * 3)] * T_z[pole * N + j]
                          + matD[IDX2D_MATDISP(mat, 2 + pole * 3)] * E_z[j];
    }
...
update_1d_electric_dispersive_axial_main_T = {
    ...
    for (pole = 0; pole < num_poles; pole++) {
        T_x[pole * N + j] -= matD[IDX2D_MATDISP(mat_x, 2 + pole * 3)] * E_x[j];
        T_y[pole * N + j] -= matD[IDX2D_MATDISP(mat_y, 2 + pole * 3)] * E_y[j];
        T_z[pole * N + j] -= matD[IDX2D_MATDISP(mat_z, 2 + pole * 3)] * E_z[j];
    }
    """
    ),
}
