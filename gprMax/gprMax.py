...
help_msg = {
    ...
    "double": (
        "(bool, opt): Flag to use double precision for compute-device (CUDA/OpenCL/Metal)"
        " solvers. Default is single precision, which provides best performance."
    ),
...
def run(
    ...
    Args:
        ...
        double: optional boolean to use double precision for compute-device
            (CUDA/OpenCL/Metal) solvers. Default is single precision.
...
def cli():
    ...
    parser.add_argument(
        "--double",
        action="store_true",
        default=args_defaults["double"],
        help=help_msg["double"],
    )
    parser.add_argument(
        "--geometry-only",
        action="store_true",
