...
class DiscretePlaneWaveAngles(GridUserObject):
    ...
    def build(self, grid: FDTDGrid):
        ...
        # Plane wave GPU support- CUDA implemented; OpenCL and Metal not yet.
...
class DiscretePlaneWaveVector(GridUserObject):
    ...
    def build(self, grid: FDTDGrid):
        ...
        # Warn about using a discrete plane wave on unsupported solvers
        # (CUDA is supported - the vector variant uses the same standard DPW
        # path as #plane_wave_angles, which runs on the GPU)
        if config.sim_config.general["solver"] in ["opencl", "metal"]:
            logger.exception(
                f"{self.params_str()} cannot currently be used "
                + "with the OpenCL or Apple Metal-based solver. "
