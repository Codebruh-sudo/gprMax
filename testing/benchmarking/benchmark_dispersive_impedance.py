...
def validate_contact(kind, directory: Path, *, iterations=2400):
    ...
    def run(label, *, python=False, resistance=None, pec=False):
        ...
        for obj in (
            ...
            gprMax.TimeWindow(iterations=iterations), gprMax.OMPThreads(1),
            gprMax.PMLThickness(thickness=0),
        ):
            scene.add(obj)
        add_exterior(scene, kind, 8*dl)
        if not pec:
            surface = (gprMax.SurfaceImpedance(id="wall", resistance=resistance)
                       if resistance is not None else gprMax.SurfaceImpedance(
                           id="wall", preset="copper", fit_frequency_range=(8e9, 12e9), fit_order=4,
                       ))
            scene.add(surface)
        scene.add(gprMax.Box(p1=(3*dl,)*3, p2=(5*dl,)*3, material_id="pec" if pec else "wall"))
        ...
        with patch.object(FDTDGrid, "build", capture), patch.object(implementation, "_cython_update", kernel):
            gprMax.run(scenes=[scene], outputfile=directory/label, cpu_precision="double",
                       hide_progress_bars=True, log_level=40)
        grid = captured["grid"]
        trace = np.asarray(grid.rxs[0].outputs["Ez"]).copy()
        fields = np.concatenate([getattr(grid, name).ravel() for name in ("Ex", "Ey", "Ez", "Hx", "Hy", "Hz")])
    ...
    relative_field_error = float(np.linalg.norm(fields-reference_fields)/max(np.linalg.norm(reference_fields), 1e-30))
    ...
    for resistance in (1e-2, 1e-3):
        ...
        pec_errors.append(float(np.linalg.norm(limit_trace-pec_trace)/np.linalg.norm(pec_trace)))
    passed = (relative_trace_error < 1e-11 and relative_field_error < 1e-9
              and tail_ratio < 0.02 and pec_errors[1] < 0.2*pec_errors[0]
              and pec_errors[1] < 1e-3)
    result = {
        "exterior": kind, "passed": bool(passed), "domain_cells": [8, 8, 8],
        "iterations": iterations, "dt_seconds": grid.dt,
        "source_frequency_hz": 10e9, "surface": "copper, four Foster poles, 8-12 GHz fit",
...
def main():
    ...
    parser.add_argument("--exteriors", nargs="+", choices=("none", "debye", "lorentz", "drude", "mixed"),
                        default=("none", "debye", "lorentz", "drude", "mixed"))
    parser.add_argument("--skip-timing", action="store_true")
    args = parser.parse_args()
    if min(args.cells, args.iterations, args.threads, args.repeats, args.kernel_iterations,
           args.kernel_repeats, args.hot_iterations, args.hot_repeats) <= 0 or args.cells < 24:
        parser.error("cells must be >=24 and iteration/thread/repeat counts must be positive")
    result = {"timings": [], "validation": [], "validation_scope": (
