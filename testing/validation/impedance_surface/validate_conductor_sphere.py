...
RADIUS = 0.016
LIMITS = dict(
    backscatter_rms_db=1.0,
    backscatter_max_db=2.0,
    complex_pattern_relative_l2=0.15,
    impedance_fit_relative_max=0.002,
...
def build_scene(dl, conductivity, threads, duration):
    ...
    wall = gprMax.SurfaceImpedance(
        ...
        fit_frequency_range=(1e9, 10e9),
        fit_order="auto",
        fit_tolerance=0.002,
    )
    transform = gprMax.NTFFFrequencyTransform(
        surface_id="surface",
        id="spectrum",
    ...
    far_field = gprMax.NTFFFarField(
        theta=np.full(ANGLES.shape, 90.0),
        phi=ANGLES,
    ...
    for obj in (
        ...
        gprMax.DiscretePlaneWaveVector(
            p1=(0.024,) * 3, p2=(0.072,) * 3, m_vec=(1, 0, 0), psi=90, waveform_id="pulse"
        ),
...
def run_case(output, dl, conductivity, args):
    ...
    if not reused:
        ...
        np.savez(
            cache.with_suffix(".npz"),
            amplitude=far_field.result.fields["Etheta"] / incident[:, None],
    ...
    fitted_z = SurfaceImpedanceModel("metal", A=wall.A, B=wall.B, C=wall.C, D=wall.D).impedance(
        FREQUENCIES
    )
    sibc_rcs, sibc_amp = sphere_reference(
        FREQUENCIES, RADIUS, conductivity, np.deg2rad(ANGLES), impedance=exact_z
    )
    ...
    metrics = dict(
        backscatter_rms_db=float(np.sqrt(np.mean(error_db**2))),
        backscatter_max_db=float(np.max(np.abs(error_db))),
        complex_pattern_relative_l2=float(
            np.linalg.norm(numerical - sibc_amp) / np.linalg.norm(sibc_amp)
        ),
        impedance_fit_relative_max=float(np.max(np.abs(fitted_z / exact_z - 1))),
        sibc_bulk_complex_relative_l2=float(
            np.linalg.norm(sibc_amp - bulk_amp) / np.linalg.norm(bulk_amp)
        ),
    )
    ...
    with (output / f"{name}.csv").open("w", newline="") as stream:
        ...
        writer.writerow(
            (
                ...
                "sibc_mie_rcs_m2",
                "bulk_mie_rcs_m2",
                "fdtd_amplitude_real_m",
                "fdtd_amplitude_imag_m",
                "sibc_amplitude_real_m",
                "sibc_amplitude_imag_m",
                "bulk_amplitude_real_m",
                "bulk_amplitude_imag_m",
            )
        )
    ...
    return dict(
        ...
        maximum_skin_depth_radius_ratio=float(
            1 / (RADIUS * np.sqrt(np.pi * FREQUENCIES[0] * mu_0 * conductivity))
        ),
        elapsed_seconds=perf_counter() - start,
        reused=reused,
    ), (rcs, sibc_rcs, bulk_rcs, numerical, sibc_amp)
...
def main(argv=None):
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path(__file__).parent / "results/conductor_sphere"
    )
    ...
    for mesh in args.mesh_mm:
        ...
        if not np.allclose(coordinates, np.round(coordinates), atol=1e-8, rtol=0):
            parser.error(
                "mesh must place domain, TFSF and NTFF boundaries and sphere centre on grid nodes"
            )
    ...
    fig, axes = plt.subplots(
        2, len(args.conductivity), figsize=(6 * len(args.conductivity), 8), squeeze=False
    )
    middle = len(FREQUENCIES) // 2
    for column, conductivity in enumerate(args.conductivity):
        selected = [
            i for i, case in enumerate(cases) if case["conductivity_s_per_m"] == conductivity
        ]
        for i in selected:
        ...
        axes[0, column].plot(
            FREQUENCIES / 1e9, 10 * np.log10(sibc[:, -1]), "k--", label="Impedance Mie"
        )
        axes[0, column].plot(
            FREQUENCIES / 1e9, 10 * np.log10(bulk[:, -1]), "r:", label="Bulk conductor Mie"
        )
        ...
        axes[0, column].set(
            title=f"Conductivity {conductivity:g} S/m; backscatter",
            xlabel="Frequency (GHz)",
            ylabel="RCS (dB m²)",
        )
        axes[1, column].set(
            title=f"Angular pattern at {FREQUENCIES[middle]/1e9:g} GHz",
    ...
    for conductivity in args.conductivity:
        ...
        if len(selected) > 1:
            convergence[str(conductivity)] = all(
                selected[-1]["metrics"][key] < selected[0]["metrics"][key]
                for key in ("backscatter_rms_db", "complex_pattern_relative_l2")
            )
    summary = dict(
        acceptance_limits=LIMITS,
        acceptance_scope="Finest mesh, each conductivity; coarse mesh is diagnostic",
        passed=all(case["passed"] for case in finest) and all(convergence.values()),
        cases=cases,
        coarse_to_fine_error_decreased=convergence,
        cpu_precision="double",
        threads=args.threads,
        fit_band_hz=[1e9, 10e9],
