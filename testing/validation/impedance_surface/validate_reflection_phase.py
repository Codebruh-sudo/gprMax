...
from .analytical import normal_reflection, discrete_normal_reflection

FREQUENCIES = np.arange(1e9, 8e9 + 0.125e9, 0.25e9)
HOSTS = {"air": dict(er_inf=1., delta_er=0., tau=80e-12),
         "debye": dict(er_inf=2.5, delta_er=2., tau=80e-12)}
DOMAIN_X, SOURCE_X, RECEIVER_X, WALL_X = 1.8, 0.8, 0.86, 0.89
LIMITS = dict(continuum_phase_rms_deg=0.1, continuum_magnitude_rms=0.002,
              discrete_phase_rms_deg=0.0002, discrete_complex_relative_l2=1e-5,
              transverse_uniformity_relative_l2=1e-9, incident_min_relative_amplitude=0.001,
...
def build_scene(dl, host, conductivity, threads, duration):
    ...
    scene = gprMax.Scene()
    for obj in (
        gprMax.Discretisation(p1=(dl,)*3), gprMax.Domain(p1=(DOMAIN_X, width, width)),
    ...
    if host == "debye":
        ...
        scene.add(gprMax.AddDebyeDispersion(poles=1, er_delta=[2.], tau=[80e-12], material_ids=["host"]))
        scene.add(gprMax.Box(p1=(0, 0, 0), p2=(DOMAIN_X, width, width), material_id="host", averaging="n"))
    wall = None
    if conductivity is not None:
        wall = gprMax.SurfaceImpedance(id="metal", conductivity=conductivity,
                                      fit_frequency_range=(0.5e9, 12e9), fit_order="auto", fit_tolerance=0.001)
        scene.add(wall)
        scene.add(gprMax.Box(p1=(WALL_X, 0, 0), p2=(WALL_X+0.004, width, width), material_id="metal", averaging="n"))
    for j in range(5):
        for k in range(4):
...
def trace(output, dl, host, conductivity, args):
    config = dict(version=2, dl=dl, host=host, host_parameters=HOSTS[host], conductivity=conductivity,
                  duration=args.duration, domain_x=DOMAIN_X, transverse_cells=4, pml_x=0,
                  source_x=SOURCE_X, receiver_x=RECEIVER_X, wall=(WALL_X, WALL_X+0.004), pulse_hz=4e9,
                  fit_band=(0.5e9, 12e9), fit_tolerance=0.001, precision="double")
...
def analyse(incident, total, dt, dl, host, wall):
    ...
    metrics = dict(continuum_phase_rms_deg=float(np.sqrt(np.mean(phase_error**2))),
                   continuum_magnitude_rms=float(np.sqrt(np.mean((np.abs(measured)-np.abs(continuum))**2))),
                   discrete_phase_rms_deg=float(np.sqrt(np.mean(discrete_phase_error**2))),
                   discrete_complex_relative_l2=float(np.linalg.norm(measured-discrete)/np.linalg.norm(discrete)),
                   transverse_uniformity_relative_l2=float(np.linalg.norm(total[0]-total[1])/np.linalg.norm(total[0])),
                   incident_min_relative_amplitude=float(np.min(np.abs(inc))/np.max(np.abs(inc))),
                   final_tenth_peak_relative=float(max(np.max(np.abs(v[:, -v.shape[1]//10:]))/np.max(np.abs(v))
                                                       for v in (incident, total))))
    passed = all(value >= LIMITS[key] if key == "incident_min_relative_amplitude" else value <= LIMITS[key]
...
def main(argv=None):
    ...
    parser.add_argument("--mesh-mm", type=float, nargs="+", default=[1., 0.5])
    ...
    parser.add_argument("--output-dir", type=Path, default=Path(__file__).parent / "results/reflection_phase")
    ...
    if not np.isfinite(args.duration) or not 0 < args.duration < earliest_return:
        parser.error(f"duration must be positive and shorter than the earliest outer return ({earliest_return:g} s)")
    for mesh in args.mesh_mm:
        if not np.isfinite(mesh) or mesh <= 0:
            parser.error("mesh spacings must be positive and finite")
        coordinates = np.array([DOMAIN_X, SOURCE_X, RECEIVER_X, WALL_X, WALL_X+0.004])/(mesh*1e-3)
        if not np.allclose(coordinates, np.round(coordinates), atol=1e-8, rtol=0):
            parser.error("mesh must place the source, receivers, wall and domain endpoints on grid nodes")
    ...
    for host in args.host:
        for mesh in sorted(set(args.mesh_mm), reverse=True):
            ...
            for conductivity in args.conductivity:
                ...
                measured, continuum, discrete, metrics, passed = analyse(incident, total, dt, mesh*1e-3, host, wall)
                name = f"{host}_sigma_{conductivity:g}_dl_{mesh:g}mm"
                case = dict(name=name, host=host, conductivity_s_per_m=conductivity, dl_m=mesh*1e-3,
                            dt_s=dt, metrics=metrics, passed=passed, elapsed_seconds=perf_counter()-start,
                            fit_poles=wall.fit_pole_count, fit_max_relative_error=wall.fit_max_relative_error)
                cases.append(case)
                curves.append((measured, continuum, discrete))
                print(json.dumps(case), flush=True)
                with (output / f"{name}.csv").open("w", newline="") as stream:
                    writer = csv.writer(stream)
                    writer.writerow(("frequency_hz", "measured_real", "measured_imag", "continuum_real", "continuum_imag",
                                     "discrete_real", "discrete_imag", "measured_phase_deg", "continuum_phase_deg", "discrete_phase_deg"))
                    writer.writerows(zip(FREQUENCIES, measured.real, measured.imag, continuum.real, continuum.imag,
                                         discrete.real, discrete.imag, *[180+np.rad2deg(np.angle(-v)) for v in (measured, continuum, discrete)]))
    ...
    for column, host in enumerate(args.host):
        for i, case in enumerate(cases):
            ...
            line, = axes[0,column].plot(FREQUENCIES/1e9, 180+np.rad2deg(np.angle(-measured)), ".", label=f"FDTD {label}")
            axes[0,column].plot(FREQUENCIES/1e9, 180+np.rad2deg(np.angle(-continuum)), color=line.get_color(), label=f"Analytical {label}")
            axes[1,column].plot(FREQUENCIES/1e9, np.abs(measured), ".", color=line.get_color())
            axes[1,column].plot(FREQUENCIES/1e9, np.abs(continuum), color=line.get_color(), label=label)
            axes[2,column].plot(FREQUENCIES/1e9, np.rad2deg(np.angle(measured/continuum)), color=line.get_color(), label=f"Continuum {label}")
            axes[2,column].plot(FREQUENCIES/1e9, np.rad2deg(np.angle(measured/discrete)), "--", color=line.get_color(), label=f"Discrete {label}")
        axes[0,column].set(title=f"{host.capitalize()} exterior; phase at wall", ylabel="Reflection phase (degrees)")
    ...
    plt.close(fig)
    summary = dict(passed=all(case["passed"] for case in cases), acceptance_limits=LIMITS, cases=cases,
                   duration_s=args.duration, frequency_band_hz=[float(FREQUENCIES[0]), float(FREQUENCIES[-1])],
                   host_parameters={host: HOSTS[host] for host in args.host},
                   earliest_outer_return_s=earliest_return, outer_return_margin_s=earliest_return-args.duration,
                   domain_x_m=DOMAIN_X, source_x_m=SOURCE_X, receiver_x_m=RECEIVER_X, wall_x_m=WALL_X,
                   cpu_precision="double", threads=args.threads, fit_band_hz=[0.5e9, 12e9],
